...
def test_variants_list_returns_paginated_rows(tmp_path: Path) -> None:
    ...
    # Three rows total; after page-1 of 2 there's still 1 row → cursor is set.
...
def test_variants_list_pagination_terminates_with_null_cursor(tmp_path: Path) -> None:
    ...
    app = build_app(derived_root=derived_root)
    with TestClient(app) as client:
        response = client.get("/v1/variants", params={"limit": 2, "offset": 2})

    assert response.status_code == 200, response.text
    body = response.json()
    # Three rows total; offset=2 returns the 3rd (last) row.
