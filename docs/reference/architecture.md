...
**`pipeline ingest` coverage-QC auto-engage** (shipped 2026-05-23, closes MVP Phase 7 AC8): when `--bam <CRAM>` is supplied without `--bed`, `pipeline ingest` (and `pipeline run`) automatically uses the bundled default gene-panel BED (`coverage_panel_default_v1.bed.gz`, ~160 genes spanning ACMG SF v3.2 + the five disease-area sysprompt panels + PharmCAT PGx genes). The mosdepth step fires against this panel; per-gene `mean_depth` + `low_coverage_exons` land in `coverage_qc`. The panel BED path and version are recorded in `coverage_qc.params_json` per INV-R001. Opt out with `--no-coverage-qc` (skips mosdepth entirely) or pass `--bed <custom.bed>` to override the default panel. The bundled panel currently ships with **placeholder exon coordinates** (training-knowledge GRCh38 chromosome positions; 8 deterministic placeholder exons per gene); real GENCODE v44 MANE Select coordinates are required before a live mosdepth run produces clinically meaningful per-exon depth — see `packages/toolkit/src/genomeclaw_toolkit/data/coverage_panel_default_v1.bed.provenance.json` for the replacement procedure.
