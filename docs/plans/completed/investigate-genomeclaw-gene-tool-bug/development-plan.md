...
**Status**: **Complete 2026-05-26**. Hypothesis #6 (agent confabulation) pinned with strong evidence — see work-notes.md "Phase 1 conclusion" section.
...
### Deliverables
1. `tests/integration/test_service_gene_endpoint_per_gene.py` — per-gene probe across failing + succeeding sets. **LANDED 2026-05-26**.
2. Inspection notes in `work-notes.md`: trace-JSON walk of Round 1-2 for confabulation evidence; direct curl probes against the running host service; reading the plugin code + the agent system prompt. **LANDED 2026-05-26**.
3. Pinned hypothesis in `work-notes.md`. **LANDED 2026-05-26** — Hypothesis #6 confirmed.

### Invariants Enforced Here
- None enforced by tests yet; diagnostic phase.

### Success Criteria
- [x] Probe test reports the actual response shape for every probed gene.
- [x] Trace-JSON walk confirms whether the agent actually invoked `genomeclaw_gene` for the failing-reported genes in Rounds 1-2.
- [x] One hypothesis (or new) is pinned with concrete evidence.

### Phase 1 outcome summary (one-liner for Phase 2 hand-off)

**Branch A (system-prompt fix)** is the right Phase 2 path. The agent's "argument-serialization bug" wording originates from the system prompt's unconditional Q-001 escape hatch at line 152 of `packages/nemoclaw-plugin/sandbox/agent-system-prompt.md`. The prompt must be tightened to (a) bind the Q-001 label to actual error evidence in the trace, and (b) instruct the agent to describe an HTTP-200 tool response on its merits (including the `region_class=null` / off-panel case) rather than falling back on a failure narrative. Phase 3's structural invariant test then catches regressions.

## Phase 2: Fix + RCA

**Status**: **Complete 2026-05-26**. Branch A (system-prompt fix) landed; new prompt contract test (`test_invA005_system_prompt_forbids_confabulated_serialization_bug_narrative`) pins the three additions; RCA brief filed at [`docs/reports/genomeclaw-gene-tool-bug-rca.md`](../../../reports/genomeclaw-gene-tool-bug-rca.md).
...
### Deliverables
1. `docs/reports/genomeclaw-gene-tool-bug-rca.md` — RCA brief. **LANDED 2026-05-26** (143 lines).
2. Code change matching the diagnosed branch (server / plugin / system-prompt). **LANDED 2026-05-26** — Branch A (system-prompt fix); new paragraph at `agent-system-prompt.md` after line 152 titled "Tool-failure narratives must match trace evidence (INV-A005)".
3. Phase 1's probe test transitions from "captures today's behaviour" to "asserts the fixed behaviour". *(Deferred to Phase 3 — the probe test is still server-side only; the behavioural-side assertion needs trace JSONs captured after Phase 3's structural enforcement landing date.)*

### Invariants Enforced Here
- INV-A001 — agent claims trace to tool output (by closing the confabulation path via prompt tightening).

### Success Criteria
- [x] Phase 1's probe test still passes (no regression in server behaviour).
- [x] No regression in existing service / plugin / agent-prompt tests (1036 passing; same 4 pre-existing failures as baseline).
- [x] RCA brief ≤ 200 lines (landed at 143 lines).
- [x] New prompt contract test passes (`test_invA005_*`).

## Phase 3: Structural Enforcement + Live Verification

**Status**: **Complete 2026-05-26** *(live re-verification deferred)*. Structural enforcement landed via `test_invA005_no_serialization_bug_confabulation.py` (parametrized over all `*.trace.json` under `docs/reports/`; date-gated to bind from 2026-05-26; all 14 historical traces skip cleanly). `INV-A005` promoted in `docs/reference/INVARIANTS.md` v1.21 (renamed from the plan-doc draft `INV-A004` because that ID was taken by `agent-decline-taxonomy-exposure`).
...
1. `test_invA005_no_serialization_bug_confabulation.py` — structural trace-walk invariant. **LANDED 2026-05-26**.
2. `INV-A005` entry in `docs/reference/INVARIANTS.md` (v1.21). **LANDED 2026-05-26**.
3. Live re-verification: re-run Q1 + Q4 against the live sandbox + capture post-fix traces. **DEFERRED** — requires `GENOMECLAW_SANDBOX_IMAGE` + `OPENAI_API_KEY` + a fresh agent turn. Acceptable per the phase doc's "live verification" section (opt-in live-LLM gate). The structural test stands ready: any post-fix trace dropped under `docs/reports/demo-2026-05-26+-logs/` activates the rule automatically.
4. Plan close-out (move active → completed). **LANDED 2026-05-26**.
