...
| Phase 7 | Pending — [phase-7.md](phases/phase-7.md) authored 2026-05-22 | | | End-to-end MVP demo + invariant sweep + the Phase-5-deferred SSRF probe via OpenShell L7 proxy under full Landlock+seccomp+netns isolation. Skeleton covers Steps 7.1 (real-data run) → 7.5 (doc drift sweep + close-out); five completion artifacts (3 live transcripts + SSRF runtime probe test + invariant sweep). |
