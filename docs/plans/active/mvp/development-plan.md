...
**Status**: Active — Phases 1–5 closed 2026-05-15; **Phase 6 COMPLETE as of 2026-05-22**: Slices A + B shipped 2026-05-15; Slice C retired; Slice E shipped via [prs-bootstrap-meta](../../completed/prs-bootstrap-meta.md) cascade (smoke v23 PRS row); **Slice D + Slice D' closed 2026-05-22** with real-data smokes: Cyrius → CYP2D6 *1/*35 (170s); PharmCAT → 9 user-applicable PGx findings (135s); **Slice F shipped 2026-05-22** with 4/4 live LLM tests passing (Story 2 + 4 + 9 + 10 against gpt-5.5 in `genomeclaw/sandbox:slice-d-prime`). **One soft warning logged** on Story 2: agent did not volunteer privacy framing — follow-up to agent system prompt, not blocking. **Phase 7 skeleton** at [phases/phase-7.md](phases/phase-7.md) ready for execution. **798 toolkit tests pass** (with sandbox image set).
**Created**: 2026-05-06
...
| Phase 7 | Pending — [phase-7.md](phases/phase-7.md) re-scoped 2026-05-22 into two close sessions | | | End-to-end MVP demo + invariant sweep + scoped SSRF probe + plan move-to-completed. **Close session 1** (~5h45m wall, ~1-2 hr foreground): Step 7.1 canonical real-data run on Nebula VCF+CRAM into one consolidated run-dir (ingest+normalize+annotate+materialize → cyp2d6-call → pharmcat → pgs-compute) + Step 7.2 invariant sweep against the run-dir + Step 7.3 re-run of 4 Slice F live tests + INV-R002 determinism diff. **Close session 2** (~2-3 hr): Step 7.4 sandbox-L7 SSRF probe (Phase-5-deferred work scoped down from full Landlock+seccomp+netns; the full kernel-level isolation probe captured as a post-MVP follow-up) + Step 7.5 final doc drift + plan move-to-completed + final commit/push. Three sub-decisions pinned 2026-05-22: full re-run (not augment); scoped SSRF (not full Landlock+seccomp+netns); reuse 4 Slice F live tests (skip Story 1 authoring). |
