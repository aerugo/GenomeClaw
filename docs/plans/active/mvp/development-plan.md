...
**Status**: Active — Phases 1–5 functionally complete; Phase 4 closed 2026-05-15 (real-data VEP smoke 4h08m58s, ClinVar parity 42,885/42,885); **Phase 5 closed 2026-05-15** (host service + plugin migration to `registerTool` + sandbox image built + live LLM round-trip verified against real OpenAI gpt-5.5 — agent invoked `genomeclaw_status`, plugin hit host service, agent surfaced fields by name in natural language. 3 sandbox-image bugs surfaced + fixed during the live sweep. One deploy-time SSRF probe deferred to Phase 7 (verifies OpenShell L7 proxy behavior under full sandbox isolation, not GenomeClaw behavior). See [phases/phase-5.md](phases/phase-5.md)). Next up: **Phase 6** — findings + evidence + lifestyle notes + Cyrius CYP2D6 + PRS.
**Created**: 2026-05-06
...
| Phase 6 | **In Progress** | 2026-05-15 | | Findings + evidence + ~~curated_notes~~ + Cyrius CYP2D6 + PRS. See [phases/phase-6.md](phases/phase-6.md). **Slices A + B shipped 2026-05-15** (15 + 9 tests). **Slice C retired** + replaced by the [agent-research-and-synthesis companion plan](../../completed/agent-research-and-synthesis/), **complete 2026-05-15** (5 slices: `live_llm` tests for Stories 4/9/10 + memory-validation-driven supersession; image bake of `IDENTITY.md` + `USER.md` + `BOOTSTRAP.md` removal + `agents.defaults.{model, thinkingDefault}`; 22 `needs_sandbox` gates; INVARIANTS.md at v1.10 — INV-A001 + INV-A002 v1.7 + INV-C001 v1.6 + INV-P001 v1.7). **Slice F partially subsumed**: Story 4 (PGx) + Story 9 (lifestyle) + Story 10 (PRS) live snapshots already shipped via the companion plan; only Story 2 ("what do you know about me?") remains. Toolkit at 570 pass / 99 skipped (host suite) + 22 `needs_sandbox` on canonical image `genomeclaw/sandbox:ars-phase-2d`. **Slice E structurally pivoted 2026-05-17** *(per [Q8 v1.6](spec.md) + [agent-driven PRS report](../../../reports/agent-driven-prs-computation.md))*: the v1.5 fixed-three-trait panel + 1-tool surface is retired; replaces with agent-driven PRS selection + 4-tool surface (plugin count 5→9). INVARIANTS.md bumped to v1.11 (`INV-A003` Agent-Curated Compute Provenance + `INV-C001` v1.7 PRS-decline pattern). The prior static-panel slice plan at [phases/phase-6-slice-e.md](phases/phase-6-slice-e.md) is **Superseded** by the v2 plan at [phases/phase-6-slice-e-v2.md](phases/phase-6-slice-e-v2.md). **Slice E.1 shipped 2026-05-17**: 5 Pydantic models, the `pgs_scores` + `pgs_compute_tasks.sqlite` schemas, 4 new host-service routes, 4 new plugin tools (count 5→9), policy preset updates. **Slice E.2 shipped 2026-05-17**: `prep/pgs.py` wrapper around `pgsc_calc` + `pipeline pgs-compute` CLI subcommand; 8 new tests (5 wrapper + 3 CLI; subprocess-mocked). The CLI subcommand also auto-INSERTs the matching `clinical-non-actionable` finding row, so the agent's `genomeclaw_findings` filter surfaces the new PRS without a second materialize step. Host suite 585 → 593 pass. **Real-data smoke** against the project owner's Nebula VCF deferred to manual (needs Nextflow + pgsc_calc + 1000G/HGDP ancestry data host-side). **Remaining**: Slice D (Cyrius CYP2D6 outside-call) + Slice E.3 (async orchestration + decline pattern + system-prompt update + sandbox image rebuild) + Slice E.4 (validation study + pre-compute consent; deferred per the methodological-review pass) + Story 2 live snapshot. |
| Phase 7 | Pending | | | End-to-end MVP demo + invariant sweep + the Phase-5-deferred SSRF probe via OpenShell L7 proxy under full Landlock+seccomp+netns isolation. |
