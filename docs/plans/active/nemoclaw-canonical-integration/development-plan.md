...
**Status**: Implementation + docs + INV-D011 all complete & committed (Phases 1–4, Phase 5 surface gate, Phase 6). The v0.4 derived-store rebuild (Phase 5 data gate) is now IN PROGRESS — colima blocker fixed; `bin/genomeclaw pipeline run` building run `2026-05-30T15-42-01Z-2b54b5` (VEP long pole), with an autonomous finisher set to bring the host service up + re-onboard + run the data-grounded smoke. Last step: verify that smoke, then move to `completed/`. See **Deferred Follow-ups**.
**Created**: 2026-05-29
...
1. **v0.4 derived-store rebuild + data-grounded smoke** (Phase 5) — **NOW RESOLVED / IN PROGRESS (2026-05-30)**. The colima blocker was fixed by adding `/Volumes/Genome_Work` to `~/.colima/default/colima.yaml` (+ bumping CPUs to 8) and restarting colima; the docker pipeline then reaches the inputs + 292GB reference and the v0.4 toolkit image has all bio tools. `bin/genomeclaw pipeline run` is building the v0.4 store (run `2026-05-30T15-42-01Z-2b54b5`; VEP is the single-core long pole). An autonomous finisher (`/tmp/genomeclaw-finisher.sh`) then brings the host service onto the new store, re-onboards the sandbox, and runs the data-grounded muscle smoke (logged to `/tmp/genomeclaw-finisher.log`). **Remaining**: verify the finisher's smoke result, then `git mv` the plan to `completed/`. Dashboard/TUI manual gates remain optional (their auth/plumbing is fixed: loopback + auth=none, canonical path).
2. **Doc-file edits** (Phase 6) — **DONE** (commit `5decf17`). The in-flight doc WIP was committed during the working-tree clustering, which unblocked clean edits: README's stale `/opt/genomeclaw`-EACCES troubleshooting + `sandbox-up.sh` description now reflect the canonical `/sandbox/build/genomeclaw` reality.
3. **INV-D011 registry entry** (Phase 6) — **DONE** (commit `5decf17`). Promoted into INVARIANTS.md v1.25 (full Rule/Why/Requirements/Where/How-to-verify + Index row), enforced by `test_invD011_plugin_install_path.py` + `test_plugin_manifest_tool_contract.py`.
4. **VEP LoF plugin degraded in the v0.4 rebuild** (toolkit follow-up, NOT this plan's plumbing). During the 2026-05-30 v0.4 pipeline run, the VEP annotate step logged repeated non-fatal warnings: `Plugin 'LoF' went wrong: Undefined subroutine &LoF::reverse_complement called at /opt/vep/.vep/Plugins/LoF.pm line 491`. VEP continues (warning, not error), so the run completes and the store is valid, but the **loss-of-function (LoF) annotation is incomplete** for affected variants (AlphaMissense + core VEP/vcfanno/ClinVar/gnomAD annotations are unaffected). Root cause: a Perl-dependency / LoF.pm version mismatch in the `genomeclaw/toolkit` image's VEP LoF plugin install (likely a missing `Bio::EnsEMBL` / LoF helper module that provides `reverse_complement`). **Action** (separate from this plan): fix the LoF plugin dependency in `packages/toolkit/Dockerfile` (the VEP/LoF install layer), rebuild `genomeclaw/toolkit`, and re-annotate; add a wrapper/probe assertion that the LoF plugin emits LoF annotations on a known LoF variant so this regresses as a typed failure (INV-T001 style) rather than a silent warning.

Once (1) lands (verify the finisher's data-grounded smoke), set Status to **Complete** and `git mv` the plan to `docs/plans/completed/nemoclaw-canonical-integration/`.
