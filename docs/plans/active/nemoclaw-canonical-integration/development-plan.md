...
| Phase 3 | In progress — Facet B ✅ + Facet A1 ✅ landed; A2 credential hand-off deferred | 2026-05-30 | | **Facet B RESOLVED** (manifest `contracts.tools` + `activation.onStartup`; agent calls tools live). **Facet A1 RESOLVED** (bake `gateway.bind=loopback` — no token, per privacy-safety review; gateway starts loopback-bound, 12/12 baked-config+manifest+path tests green on rebuilt `:phase3` image). **A2 = FEASIBLE** (investigated 2026-05-30): intended OpenShell design is placeholder + L7-proxy credential rewriting (gateway holds a placeholder, proxy attaches the real registered credential at egress — `onboard.js:4150-4152`), so the gateway needs no raw key in env and `nemoclaw recover` works. Implementation = rely on native provider flow + inference.local placeholder; delete onboard Step 6 (literal-key auth-profiles.json) + Step 7b; verify native launch + agent LLM turn without `-e OPENAI_API_KEY`. Research: [initial_findings.md](initial_findings.md), work-notes "A2 feasibility". |
| Phase 4 | Pending | | | Script simplification + recovery |
| Phase 5 | Pending | | | Verification gate |
